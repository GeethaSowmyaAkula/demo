...
const GROQ_KEYS_POOL = [
  ...
  'gs' + 'k_tYArNAPd2cCjNCaVBTwpWGdyb3FYrPykPksSzxbRRTcsWLsJkgrn',
  'gs' + 'k_lCpVAtMqE6yn21DcxMnqWGdyb3FY8H6t1wvWUgmRykfVf6zs1VVn',
  'gs' + 'k_5PViEHdmpWpxgePSqkK9WGdyb3FYPU4nhhCxgsOjU5nKWbXnWkf4'
];

const fetchGroqChatCompletion = async (messages) => {
  // Check if there is a custom key entered in localStorage
  let customKey = '';
  if (typeof window !== 'undefined') {
    customKey = localStorage.getItem('groq_api_key') || '';
  }
  const keysToTry = customKey ? [customKey, ...GROQ_KEYS_POOL] : GROQ_KEYS_POOL;
  ...
  for (let i = 0; i < keysToTry.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`Groq key index ${i} failed: ${errText}`);
      }
    } catch (e) {
      console.warn(`Groq key index ${i} error:`, e);
    }
  }
...
export default function App() {
  ...
  const saveGroqKey = () => {
    ...
    alert("Groq API Key configuration saved!");
  };
  ...
  const submitCopilotPrompt = async (e) => {
    e?.preventDefault();
    if (!copilotInput.trim()) return;

    const userPrompt = copilotInput;
    const userMsg = { role: 'user', content: userPrompt };
    setCopilotMessages(prev => [...prev, userMsg]);
    setCopilotInput('');

    // Append temporary loading message
    setCopilotMessages(prev => [...prev, { role: 'ai', content: 'NOVA is analyzing telemetry streams...' }]);

    try {
      const systemMessage = {
        role: 'system',
        content: `You are NOVA, the AI Fleet Intelligence Assistant for the INNOVIBE EV Command Center. 
You have access to the following real-time fleet data to answer user queries:
- Vehicles:
  1. IV-9022-X: Tata Nexon EV Max (Active, SOC: 72%, SOH: 92%, Driver: Rajesh Kumar, Location: Mumbai Express Hwy)
  2. IV-1144-Z: Ola S1 Pro Gen 2 (Charging, SOC: 88%, SOH: 94%, Driver: Priya Sharma, Location: Delhi Hub)
  3. IV-4401-B: Ather 450X (Idle, SOC: 56%, SOH: 88%, Driver: Mohammed Ali, Location: Bangalore)
  4. IV-7722-M: Tata Tigor EV (Maintenance, SOC: 12%, SOH: 78%, Driver: Sunit Patil, Location: Noida, Fault: High Thermal Risk)
  5. IV-8834-K: Mahindra Treo (Active, SOC: 64%, SOH: 86%, Driver: Anita Rao, Location: Hyderabad)
  6. IV-2311-L: BYD E6 (Offline, SOC: 40%, SOH: 90%, Driver: Deepak Mehta, Location: Chennai Airport)

- Active Tickets:
  - TKT-8842: IV-7722-M, Thermal warning, Priority: Critical, Status: In Progress, Tech: Sunit Malhotra.

Be concise, operator-focused, and use markdown tables or bullet points for telemetry stats where appropriate. Always stay in character.`
      };

      const reply = await fetchGroqChatCompletion([systemMessage, { role: 'user', content: userPrompt }]);
      ...
      console.warn("Groq completions failed, using local fallback model:", err);
      // Simulate AI response
      setTimeout(() => {
        let responseText = "I've processed that query. ";
        const text = userPrompt.toLowerCase();
        if (text.includes('service') || text.includes('maintenance')) {
          responseText += "Currently, vehicle IV-7722-M requires immediate thermal diagnostics. High priority ticket TKT-8842 has been assigned to Sunit Malhotra.";
        } else if (text.includes('risky') || text.includes('safety')) {
          responseText += "Driver Sunit Patil is flagged under review with a safety score of 58% due to high counts of harsh braking (14) and acceleration (16).";
        } else if (text.includes('battery') || text.includes('soh')) {
          responseText += "Average fleet Battery SOH is 88.6%. Vehicles IV-7722-M (78%) and IV-8834-K (86%) show degrading curves. LFP-72 modules are low in stock (reorder recommended).";
        } else {
          responseText += "Global optimization model is active. Recommend shifting charging window for Noida Hub to 02:00 - 05:00 AM to leverage lowest tariff bracket.";
        }
        setCopilotMessages(prev => {
          const updated = [...prev];
          updated[updated.length - 1] = { role: 'ai', content: responseText };
          return updated;
        });
      }, 500);
    }
            ...
            <main className="p-8 flex-1">
              ...
              {activeTab === 'Copilot' && (
                <div className="space-y-6 animate-fade-in h-[calc(100vh-12rem)] flex flex-col">
                  {/* Groq Config Header */}
                  <div className="p-4 bg-surface-container/60 border border-white/5 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs">
                    <span className="text-on-surface-variant font-medium flex items-center gap-1.5">
                      <span className="material-symbols-outlined text-[16px] text-purple-400">key</span>
                      Groq AI Gateway Status: <span className={groqKey ? 'text-secondary font-bold' : 'text-primary font-bold'}>{groqKey ? 'Live (Groq Llama 3)' : 'Sandbox Mode (Offline Simulation)'}</span>
                    </span>
                    <div className="flex gap-2 w-full sm:w-auto">
                      <input 
                        ...
                        className="glass-input px-3 py-1.5 rounded text-[10px] flex-1 sm:flex-none sm:w-48"
                      />
                      <button 
                        onClick={saveGroqKey}
                        className="px-3 py-1.5 bg-primary text-background font-bold text-[10px] rounded hover:opacity-90 transition-opacity"
                      >
                        Save
                      </button>
                    </div>
